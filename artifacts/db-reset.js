...
MongoClient.connect(db, (err, db) => {
  ...
  Promise.all(dropPromises).then(() => {
    ...
    usersCol.insertMany(USERS_TO_INSERT, (err, data) => {
      ...
      // Begin inserting savings
      console.log("Savings to insert:");
      totalSavings.forEach((saving) => console.log(JSON.stringify(saving)));

      savingsCol.insertMany(totalSavings, (err, data) => {
        parseResponse(err, data, "savingsCol.insertMany");
